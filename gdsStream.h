...
static const char* gdsStream_hwhat = "@(#) $RCSfile: gdsStream.h,v $ $Revision: 1.1 $ $Date: 2005-04-18 00:13:52-05 $";
...
protected:
    ...
    char*   CurrentStrName;
    time_t  Time_val;
    char    Record[20480];  //10x 2048 for large boundaries ...
...
public:
    ...
    void    wrstrm(char record[20480], GDSFILE* gdsfile);
            // write external record to stream file, Rectyp,Dattyp,Length taken from args
    void    wrstrm(char record[20480], int Rectyp, int Dattyp, int Length);
    ...
    int     iround(int number, int places);
} //end class GDSFILE
...
#define         BOX             45    // NO_DATA    The beginning of a BOX element.
...
#define         LIBSECUR        59    // INTEGER_2  Access control list stuff for CalmaDOS, ancient. INFORM used this when creating a new library. Had 1 to 32 entries with group numbers, user numbers and access rights.
