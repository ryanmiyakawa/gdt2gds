...
static const char* gdsStream_Cwhat = "@(#) $Id: gdsStream.C $ $Revision: 8 $ $Date: 2006-08-17 04:03:07 -0500 (Fri, 17 Aug 2006) $";
...
) 
{
    ...
    Rectyp = XY;
    Dattyp = INTEGER_4;
    putI32(iround((int)(x1 * factor),1)/10, 0);
    putI32(iround((int)(y1 * factor),1)/10, 4);
    putI32(iround((int)(x2 * factor),1)/10, 8);
    putI32(iround((int)(y2 * factor),1)/10, 12);
    putI32(iround((int)(x3 * factor),1)/10, 16);
    putI32(iround((int)(y3 * factor),1)/10, 20);
    wrstrm();
       ...
       double dbu_uu)
{
    ...
    Rectyp = XY;
    Dattyp = INTEGER_4;
    putI32(iround((long)(minX * factor),1)/10,  0);   // lower left corner
    putI32(iround((long)(minY * factor),1)/10,  4);
    putI32(iround((long)(minX * factor),1)/10,  8);   // upper left corner
    putI32(iround((long)(maxY * factor),1)/10, 12);
    putI32(iround((long)(maxX * factor),1)/10, 16);   // upper right corner
    putI32(iround((long)(maxY * factor),1)/10, 20);
    putI32(iround((long)(maxX * factor),1)/10, 24);   // lower right corner
    putI32(iround((long)(minY * factor),1)/10, 28);
    putI32(iround((long)(minX * factor),1)/10, 32);   // lower left corner again
    putI32(iround((long)(minY * factor),1)/10, 36);
    wrstrm();
          ...
          double dbu_uu)
{
    ...
    Rectyp = XY;
    Dattyp = INTEGER_4;
    putI32(iround((long int)(x_coord * factor),1)/10, 0);
    putI32(iround((long int)(y_coord * factor),1)/10, 4);
    wrstrm();
                      ...
                      double dbu_uu)
{
    ...
    Rectyp = XY;
    Dattyp = INTEGER_4;
    putI32(iround((int)(x * factor),1)/10, 0);
    putI32(iround((int)(y * factor),1)/10, 4);
    wrstrm();
                      ...
                      double dbu_uu)
{
    ...
    fstep =  (float) width * factor; // done in steps because of previous compilier bug
    istep =  (long) fstep;
    if (istep)
    {
        // WRITE WIDTH
...
int GDSFILE::putBndDbl(int layer, int datatyp, double xArray[], double yArray[], int nVert, double dbu_uu)
{
    ...
    {
        fstep =  (float) xArray[index] * factor; // done in steps because of previous compilier bug
        istep =  (long) fstep;
        putI32(iround(istep, 1)/10, (index*8));
        fstep =  (float) yArray[index] * factor;
        istep =  (long) fstep;
        putI32(iround(istep, 1)/10, (index*8) + 4);
    }
    if (xArray[0] != xArray[nVert - 1] && yArray[0] != yArray[nVert - 1]) 
    {
        fstep =  (float) xArray[0] * factor; // done in steps because of previous compilier bug
        istep =  (long) fstep;
        putI32(iround(istep, 1)/10, (index * 8));
        fstep =  (float) yArray[0] * factor;
        istep =  (long) fstep;
        putI32(iround(istep, 1)/10, (index * 8) + 4);
        Length += 8;
    }
...
int GDSFILE::putPathDbl(int layer, int datatyp, int pathtyp, double width, double bgnextn, double endextn, double xArray[], double yArray[], int nVert, double dbu_uu)
{
    ...
    if (pathtyp == 4)
    {
        ...
        Rectyp = BGNEXTN;
        Dattyp = INTEGER_4;
        fstep =  (float) bgnextn * factor; // done in steps because of previous compilier bug
        istep =  (long) fstep;
        putI32(iround(istep, 1)/10, 0);
        wrstrm();
        ...
        Rectyp = ENDEXTN;
        Dattyp = INTEGER_4;
        fstep =  (float) endextn * factor; // done in steps because of previous compilier bug
        istep =  (long) fstep;
        putI32(iround(istep, 1)/10, 0);
        wrstrm();
    }
